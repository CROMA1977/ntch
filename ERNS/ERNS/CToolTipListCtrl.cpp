...
void CToolTipListCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	// TODO: 在此加入您的訊息處理常式程式碼和 (或) 呼叫預設值
	DWORD   dwPos = GetMessagePos();
	CPoint   point1(LOWORD(dwPos), HIWORD(dwPos));
	this->ScreenToClient(&point1);

	LVHITTESTINFO   lvinfo;
	lvinfo.pt = point1;
	lvinfo.flags = LVHT_ABOVE;

	int nItem = this->SubItemHitTest(&lvinfo);
	if (nItem != -1) {
		if (this->m_CERNSDlg != NULL) {
			CERNSDlg *ptr = dynamic_cast<CERNSDlg *>(this->m_CERNSDlg);
			if(ptr != NULL)
				ptr->OutputList(lvinfo.iItem, lvinfo.iSubItem);
		}
	}
